...
5. Click **Deploy**. First boot will install `requirements.txt` and
   run the Supabase PostgreSQL schema once in the SQL Editor.
...
Streamlit Community Cloud's filesystem is **ephemeral** — persistent application data is therefore stored in Supabase
...
- [x] Supabase PostgreSQL + Storage is the application persistence layer.


## EcoVision Supabase migration

Run `database/supabase_schema.sql` in Supabase before deploying. Do not deploy the old SQLite schema.
...
Google sign-in additionally requires the `[auth]` Streamlit OIDC block documented in `README.md`.
