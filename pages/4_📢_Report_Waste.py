...
from database.db import upload_media, SupabaseUnavailableError
    ...
    with st.spinner("Submitting complaint..."):
        ...
        try:
            if media_source == "video" and video_bytes_for_save:
                ...
                safe_name = re.sub(r"[^A-Za-z0-9._-]", "_", media_name or "upload.mp4")
                storage_path = f"complaints/{user['id']}/{timestamp}_{safe_name}"
                image_path = upload_media(video_bytes_for_save, storage_path, video_file.type if video_file else "video/mp4")
            elif media_bytes:
                ext = Path(media_name).suffix or ".jpg"
                safe_name = re.sub(r"[^A-Za-z0-9._-]", "_", media_name or "capture.jpg")
                storage_path = f"complaints/{user['id']}/{timestamp}_{safe_name}"
                image_path = upload_media(media_bytes, storage_path, media_mime)
        except SupabaseUnavailableError as exc:
            st.warning(f"Media could not be uploaded to Supabase Storage. The complaint will still be saved without the attachment. Details: {exc}")
