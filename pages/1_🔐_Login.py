...
if hasattr(st, "user") and getattr(st.user, "is_logged_in", False) and not st.session_state.get("user"):
    try:
        ...
        st.session_state["user"] = upsert_google_user(dict(st.user))
        st.switch_page("pages/7_🧑‍💼_Officer_Dashboard.py" if st.session_state["user"].get("role") == "officer" else "pages/3_🏠_Citizen_Dashboard.py")
...
with g2:
    st.caption("Or continue with your EcoVision email and password.")
st.divider()
