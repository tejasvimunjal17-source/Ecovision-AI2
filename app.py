...
Landing page is public and intentionally has no Streamlit sidebar. Once a
user authenticates, the app routes directly to the appropriate dashboard.
"""
import streamlit as st
from database.db import init_db
from utils.helpers import load_css, init_session_state
from config import settings
from backend.auth import upsert_google_user
...
# Streamlit's native OIDC flow. Configure [auth] in Streamlit Secrets for
# Google. The app still works with normal email/password authentication.
if hasattr(st, "user") and getattr(st.user, "is_logged_in", False) and not st.session_state.get("user"):
    try:
        claims = dict(st.user)
        st.session_state["user"] = upsert_google_user(claims)
        st.session_state["nav_page"] = "🏠 Dashboard"
        st.rerun()
    except Exception as exc:
        st.error(f"Google sign-in could not be completed: {exc}")

# Authenticated users should never see the public landing page again.
if st.session_state.get("user"):
    role = st.session_state["user"].get("role", "citizen")
    target = {
        "citizen": "pages/3_🏠_Citizen_Dashboard.py",
        "officer": "pages/7_🧑‍💼_Officer_Dashboard.py",
        "admin": "pages/8_🛠️_Admin_Dashboard.py",
    }.get(role, "pages/3_🏠_Citizen_Dashboard.py")
    st.switch_page(target)

load_css(public=True, show_chat=False)

# ---------------------------------------------------------------
# Top navigation — public landing only
# ---------------------------------------------------------------
    ...
    with c1:
        if st.button("🔐 Continue with Google", use_container_width=True, type="primary"):
            if hasattr(st, "login"):
                ...
                st.error("Google sign-in requires a newer Streamlit version.")
    with c2:
...
# ---------------------------------------------------------------
# HERO — original EcoVision content retained
# ---------------------------------------------------------------
st.markdown(
    """
    <div class="eco-hero">
        ...
        <div style="margin-top:1rem;opacity:.78;">Secure sign-in · Supabase PostgreSQL · AI-powered civic sustainability</div>
    </div>
    """,
    unsafe_allow_html=True,
)

auth_c1, auth_c2, auth_c3 = st.columns([1, 1, 1])
with auth_c1:
    if st.button("🚀 Create Account", use_container_width=True, type="primary"):
        st.switch_page("pages/2_📝_Register.py")
with auth_c2:
    if st.button("✉️ Sign in with Email", use_container_width=True):
        st.switch_page("pages/1_🔐_Login.py")
with auth_c3:
    st.page_link("pages/9_🤖_Prakriti_AI_Connect.py", label="🌿 Talk to Prakriti AI", use_container_width=True)

st.markdown("---")
state["_scroll_features"] = True
...
st.page_link("pages/8_🛠️_Admin_Dashboard.py", label="🛡️ Admin Portal", icon="🛡️")
